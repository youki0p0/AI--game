...
simple_bot」から **1手読み探索** に置き換えたもの。各決定で *全ての合法オプション* を
エンジンの探索サンドボックス(search_begin/search_step)へ適用し、結果盤面を静的評価
(state_eval: サイド差>>マテリアル>脅威>資源、将棋流)で採点して最善手を選ぶ。

実測(同一相手=simple_bot操縦の実メタ, N=50/相手, 計250戦):
  探索 OVERALL 0.752 [0.698,0.806] vs simple_bot 0.656 [0.597,0.715]（+0.10）。
  接戦マッチが全て改善: vs Grimmsnarl 0.58→0.72 / vs Alakazam 0.46→0.70 / vs MegaStarmie 0.48→0.58。

隠れ情報は自デッキ(ARCH_DECK)から再構成。相手デッキ未知でも 1手読み(自ターン打ち切り)は
...
# 探索パラメータ
N_TURNS = 1            # 1手読み(自ターン打ち切り)。相手デッキ未知に頑健。
